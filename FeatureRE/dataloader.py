...
def get_dataloader_partial_split(opt, train_fraction=0.1, train=True, pretensor_transform=False,shuffle=True,return_index = False):
    ...
    if opt.dataset == "gtsrb":
        ...
        dataset = torchvision.datasets.SVHN(opt.data_root, split='train' if train else 'test', transform=transform_train, download=True)
        dataset_test = torchvision.datasets.SVHN(opt.data_root, split='train' if train else 'test', transform=transform_test, download=True)
        class_num=10
    elif opt.dataset == "mnist":
