...
def get_transform(opt, train=True, pretensor_transform=False):
    ...
    if opt.dataset == "cifar10":
        # Match project training (config.DATASET_CONFIGS['CIFAR10']); not the 0.247/... tuple used in some papers.
        ...
        raise Exception(f"Invalid Dataset: {opt.dataset}")
...
def get_dataloader_partial_split(opt, train_fraction=0.1, train=True, pretensor_transform=False,shuffle=True,return_index = False):
    ...
    if opt.dataset == "imagenet_sub200":
        ...
        if opt.dataset == "cifar10":
            ...
            dataset = torchvision.datasets.SVHN(opt.data_root, split=split, transform=transform_train, download=True)
            dataset_test = torchvision.datasets.SVHN(opt.data_root, split=split, transform=transform_test, download=True)
            class_num=10
        elif opt.dataset == "gtsrb":
            split = 'train' if train else 'test'
            gtsrb_root = os.path.join(opt.data_root, 'gtsrb')
            dataset = torchvision.datasets.GTSRB(gtsrb_root, split=split, transform=transform_train, download=True)
            dataset_test = torchvision.datasets.GTSRB(gtsrb_root, split=split, transform=transform_test, download=True)
            class_num=43
