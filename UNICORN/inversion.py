...
from models import nin,vgg,resnet,wresnet,inception,densenet,mobilenetv2,efficientnet,convnet


class TimmViTAdapter(nn.Module):
    """Adapter exposing UNICORN's expected feature IO methods for timm ViT."""
    ...
    def from_input_to_features(self, x, index):
        ...
        # timm ViT may return [B, D] or token sequence [B, N, D].
        if feats.dim() == 3:
            feats = feats[:, 0]
        return feats

    def from_features_to_output(self, x, index):
        # from_input_to_features uses cls token only -> [B, D]. timm forward_head expects [B, N, D].
        if x.dim() == 2:
            return self.model.head(x)
        if hasattr(self.model, "forward_head"):
            return self.model.forward_head(x, pre_logits=False)
        if x.dim() == 3:
            x = x[:, 0]
        return self.model.head(x)


def load_timm_vit_adapter_state_dict(adapter: TimmViTAdapter, state_dict: dict) -> None:
    """
    Load weights into TimmViTAdapter. Checkpoints from timm / this repo use flat keys
    (cls_token, blocks.0..., head.weight); those belong on adapter.model, not on the
    wrapper (which would expect model.* prefixes).
    """
    stripped = {}
    ...
    if stripped and any(k.startswith("model.") for k in stripped):
        try:
            adapter.load_state_dict(stripped, strict=True)
        except RuntimeError as e:
            print(f"Warning: Could not load ViT adapter state_dict strictly. Error: {e}")
            adapter.load_state_dict(stripped, strict=False)
        return

    inner = adapter.model
    try:
        inner.load_state_dict(stripped, strict=True)
    except RuntimeError as e:
        print(f"Warning: Could not load ViT timm module strictly. Error: {e}")
    ...
    def _get_classifier(self, opt):
        ...
        if opt.model_path:
            ...
            if isinstance(state_dict, dict):
                ...
                elif "net_state_dict" in state_dict:
            ...
            is_convnet_ckpt = all(
                key in checkpoint_keys for key in ["conv1.weight", "conv2.weight", "fc1.weight", "fc2.weight"]
            )
            is_vit_ckpt = (
                "patch_embed.proj.weight" in checkpoint_keys and
                ("head.weight" in checkpoint_keys or any(k.startswith("blocks.") for k in checkpoint_keys))
            )
            ...
            if is_convnet_ckpt:
                ...
                patch_embed_weight = actual_state_dict["patch_embed.proj.weight"]
                input_channels = int(patch_embed_weight.shape[1])
                num_classes = int(actual_state_dict["head.weight"].shape[0]) if "head.weight" in actual_state_dict else opt.num_classes
                print(f"Detected ViT architecture: input_channels={input_channels}, num_classes={num_classes}")
                ...
                classifier = TimmViTAdapter(vit_model).to(opt.device)
                load_timm_vit_adapter_state_dict(classifier, actual_state_dict)
            else:
                ...
                if opt.arch:
                    ...
                    if opt.arch == "vit":
                        in_channels = getattr(opt, "input_channel", 3)
                ...
                if classifier is None:
                    raise ValueError(f"No architecture specified and could not auto-detect from checkpoint. Available arch options: {opt.arch if hasattr(opt, 'arch') else 'None'}")
                
                # Try to load the state dict
                if isinstance(classifier, TimmViTAdapter):
                    load_timm_vit_adapter_state_dict(classifier, actual_state_dict)
                else:
                    try:
                        classifier.load_state_dict(actual_state_dict)
...
def train(opt, init_mask, init_pattern):
    ...
    os.makedirs("./encoder_decoder_re_imgs", exist_ok=True)
                ...
                if "wanet" in opt.model_path:
                    ...
                    for j in range(GT_img.shape[0]):
                        ...
                        if "filter" in opt.model_path:
                            ...
                            # Default case: use original image if no specific attack type is detected
                            convered_pil_img = ori_pil_img
